...
- **Standards Compliance Linter** -- `make lint-standards` checks commits, secrets, coverage, type safety, and banned functions. Automated compliance checks currently ship for **Python, TypeScript, Go, and Elixir** (plus common cross-language checks); the other nine language standards are documented but not yet machine-enforced (see [scripts/lint-checks/README.md](scripts/lint-checks/README.md)). Outputs text, JSON, or SARIF for GitHub Code Scanning.
